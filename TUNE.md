...
Stat icons (`stat-constitution.png` and friends) are unused while Character Level is out of the app. Slot and weapon icons still show.
...
- `scripts/overlay_logic.js` — set-name overlays and leftover Level math.
...
- Every jump (`0.11` → `0.12`) gets a frozen branch `archive/vX.Y.Z` **before** the next jump.
- Character Level is frozen at `archive/v0.10.0` and `archive/v0.11.0`. We can put it back later without losing that work.
